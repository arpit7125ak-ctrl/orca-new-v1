...
```jsonc
{
  ...
  "data": {
    ...
    "points": [                                             // sampled points, see PointObservation below
      { "point_id": "P1", "lat": 13.0827, "lon": 80.5, "point_status": "applicable", "land_sea": "sea" }
    ]
  }
}
```
...
**`PointObservation` required fields:** `point_id`, `lat`, `lon`, `point_status`, `land_sea`. `point_status` enum is `applicable|not_applicable|prohibited`. A land point inside a valid sampling grid is `not_applicable` — that's correct, not an error, and the Backend expects it (never drop the point from the array).
