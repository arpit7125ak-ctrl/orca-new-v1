...
export default function QueryForm({ onSubmit, isLoading, selectedLang }) {
  ...
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-2xl p-4 sm:p-6 shadow-xl backdrop-blur-sm">
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Main Input Bar */}
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-xs sm:text-sm font-semibold text-[var(--text-primary)] flex items-center space-x-1.5">
              <Sparkles className="w-4 h-4 text-[var(--accent-primary)]" />
              <span>{t('home.title')}</span>
            </label>
            <span className="text-[11px] text-[var(--text-secondary)]">{t('home.subtitle')}</span>
          </div>
          ...
          <div className="relative flex items-center">
            <input
              ...
              disabled={isLoading}
              className="w-full pl-4 pr-24 sm:pr-28 py-3.5 bg-[var(--bg-base)] border border-[var(--border-base)] rounded-xl text-sm sm:text-base text-[var(--text-primary)] placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all shadow-inner"
            />
            ...
            <div className="absolute right-2 flex items-center space-x-1.5">
              ...
              <button
                ...
                className={`p-2 rounded-lg transition-all ${
                  isRecording
                    ? 'bg-[var(--dangerous)] text-[var(--text-primary)] animate-pulse ring-2 ring-rose-400/50'
                    : 'bg-[var(--bg-surface-2)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-surface-2)]'
                }`}
              ...
              <button
                ...
                className="px-3.5 py-2 bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-hover)] hover:from-cyan-400 hover:to-blue-500 text-black font-bold rounded-lg text-sm flex items-center space-x-1.5 disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-lg transition-all"
              >
          ...
          {isRecording && (
            <p className="text-xs text-[var(--dangerous-bright)] mt-1.5 font-medium animate-pulse flex items-center space-x-1">
              <span className="w-2 h-2 rounded-full bg-[var(--dangerous)] inline-block" />
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              className="text-xs text-[var(--accent-primary)] hover:text-cyan-300 flex items-center space-x-1 font-medium"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {presets.map((p, idx) => (
              <button
                ...
                className="text-left p-2.5 rounded-xl bg-[var(--bg-base)] hover:bg-slate-800/80 border border-[var(--border-base)] hover:border-cyan-500/40 transition-all group"
              >
                <div className="text-xs font-semibold text-[var(--text-primary)] group-hover:text-cyan-300 truncate">
                  {p.title}
                </div>
                <div className="text-[10px] text-[var(--text-secondary)] truncate mt-0.5">{p.detail}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        {showAdvanced && (
          <div className="pt-3 border-t border-[var(--border-base)] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 bg-[var(--bg-base)] p-3 rounded-xl">
            {/* Activity */}
            <div>
              <label className="block text-[11px] font-semibold text-[var(--text-secondary)] mb-1 flex items-center space-x-1">
                <Fish className="w-3 h-3 text-[var(--accent-primary)]" />
                <span>{t('home.maritimeActivity')}</span>
              </label>
              <select
                value={activity}
                onChange={(e) => setActivity(e.target.value)}
                className="w-full bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-lg px-2.5 py-1.5 text-xs text-[var(--text-primary)] focus:ring-1 focus:ring-cyan-500"
              >
            ...
            <div>
              <label className="block text-[11px] font-semibold text-[var(--text-secondary)] mb-1 flex items-center space-x-1">
                <Ship className="w-3 h-3 text-[var(--accent-primary)]" />
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="text-[11px] font-semibold text-[var(--text-secondary)] flex items-center space-x-1">
                ...
                <button
                  ...
                  className="text-[10px] text-[var(--accent-primary)] hover:underline flex items-center space-x-0.5"
                >
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <input
                  ...
                  className="bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-primary)]"
                />
                <input
                  type="number"
                  step="0.0001"
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-[var(--bg-surface)] border border-[var(--border-base)] rounded-lg px-2.5 py-1.5 text-xs text-[var(--text-primary)]"
              />
            </div>
          </div>
        )}
