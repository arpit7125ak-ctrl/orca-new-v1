    ...
    if (this.state.hasError) {
      ...
      return (
        <div className="min-h-screen bg-[var(--bg-base)] text-[var(--text-primary)] flex items-center justify-center p-6">
          <div className="max-w-lg w-full bg-[var(--bg-surface)] border border-rose-800/80 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-5 text-center">
            <div className="w-14 h-14 bg-[var(--dangerous)]/20 text-[var(--dangerous-bright)] border border-rose-700/60 rounded-2xl flex items-center justify-center mx-auto shadow-lg shadow-rose-950/50">
              <AlertTriangle className="w-8 h-8 animate-bounce" />
            </div>

            <div>
              <h2 className="text-xl font-black text-[var(--text-primary)]">{t('error.boundaryTitle', { ns: 'ui' })}</h2>
              <p className="text-xs text-[var(--text-secondary)] mt-1">
                {t('error.boundarySubtitle', { ns: 'ui' })}
              </p>
            </div>

            <div className="bg-[var(--bg-base)] border border-[var(--border-base)] rounded-xl p-3.5 text-left text-xs font-mono text-[var(--dangerous-bright)] overflow-x-auto max-h-40">
              {this.state.error?.toString() || t('error.unknownRuntime', { ns: 'ui' })}
            </div>

            <button
              onClick={this.handleReload}
              className="inline-flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-[var(--accent-primary)] hover:bg-[var(--accent-hover)] text-black font-bold text-xs transition-all shadow-lg shadow-lg"
            >
