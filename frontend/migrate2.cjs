...
files.forEach(file => {
  ...
  // Since we only want to replace tailwind classes inside className="...", we can just replace space-or-quote bounded strings.
  // Using .split and .join with spaces doesn't work if they are next to quotes.
  // Using a simpler regex that covers bounds: `(?:^|\\s|"|'|\`)(target)(?=$|\\s|"|'|\`)`
